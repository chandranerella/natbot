...
if (
	...
	def get_gpt_command(objective, url, previous_command, browser_content):
		...
		response = client.chat.completions.create(
			model="gpt-3.5-turbo",  # Or "gpt-4" based on your requirement
			messages=[
				{"role": "system", "content": "You are a helpful assistant."},
				{"role": "user", "content": prompt},  # Replace `prompt` with your actual content
			],
			temperature=0.5,
			n=3,  # Equivalent to generating multiple completions
			max_tokens=50,
		)
